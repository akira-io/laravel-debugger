    ...
    public function send(Request $request): void
    {
        ...
        foreach ($requestProperties['payloads'] as &$payload) {
            ...
            if (isset($payload['content']['values'])) {
                $payload['content']['values'] = preg_replace('/sf-dump-\d{1,10}/', 'sf-dump-xxxxxxxxxx', $payload['content']['values']);
            }

            if (isset($payload['content']['attributes'])) {
                $payload['content']['attributes'] = preg_replace('/sf-dump-\d{1,10}/', 'sf-dump-xxxxxxxxxx', $payload['content']['attributes']);
            }
